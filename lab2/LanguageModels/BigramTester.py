...
class BigramTester(object):
    ...
    def compute_entropy_cumulatively(self, word):
        ...
        if word in self.index:
            if self.bigram_exists(word):
                prob += self.lambda1*np.power(np.e, self.bigram_prob[self.last_index][self.index[word]])
            self.last_index = self.index[word]
            prob += self.lambda2*self.unigram_count[self.index[word]]/self.total_words
        else:
            self.last_index = -1

        prob += self.lambda3
        self.test_words_processed += 1
        self.logProb = (self.logProb * (self.test_words_processed - 1) + -1 * np.log(prob)) / self.test_words_processed

    def bigram_exists(self, word):
        return self.last_index != 1 and self.index[word] in self.bigram_prob[self.last_index]
